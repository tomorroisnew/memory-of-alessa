...
#include "Effect2/hh_class_water_14.h"
...
static u_int Object_Initialize(HH_Object_Water_14* pThis /* r16 */) {
    u_int result = 1; // r2
    pThis->Timer = 0.0f;
    pThis->Motion_Step = 0;
    ...
    _Area_Info_List_0x0036FBF0->pGrid_Y_Value = pThis->Area00_Grid_Y_Value;
    return result;
}
...
static u_long _GifTag[2] = {
    ...
    GIF_REG(SCE_GIF_PACKED_AD, 0) | GIF_REG(SCE_GS_PRIM, 1) | GIF_REG(SCE_GS_PRIM, 2) | GIF_REG(SCE_GS_PRIM, 3)}; // @ 0x0036FC90
...
static u_int Object_Draw(HH_Object_Water_14* pThis /* r22 */, float* pGrid_Y_Value /* r21 */, float* WorldLocation /* r17 */, u_int Grid_X_Max /* r20 */, u_int Grid_Z_Max /* r18 */) {
    static float Light_Base[4] = {16.0f, 16.0f, 16.0f, 0.0f};       // @ 0x0036FCB0
    ...
    for (z_index = 0; z_index < z_grid_max; z_index++) {
        ...
        sceVu0FVECTOR Grid_Vertex2 = {100.0f, 0.0f, 100.0f * z_index, 1.0f};       // r29+0x200
        ...
        for (x_index = 0; x_index < Grid_X_Max; x_index++) {
            ...
            static float sx_122 = 2.5f; // @ 0x0036FD30
            static float ty_123 = 2.5f; // @ 0x0036FD38

            Grid_Vertex0[0] = Grid_Vertex1[0] = 100.0f * x_index;

            Grid_Vertex2[0] = ((x_index + 1) % x_grid_max) * 100.0f;
            Grid_Vertex0[1] = pGrid_Y[HH_WATER_14_GRID_INDEX_GET(x_index, z_index)];
            Grid_Vertex1[1] = pGrid_Y[HH_WATER_14_GRID_INDEX_GET(x_index, z_index + 1)];
            Grid_Vertex2[1] = pGrid_Y[HH_WATER_14_GRID_INDEX_GET((x_index + 1) % x_grid_max, z_index)];

            base = 0.25f * (1.0f / (100.0f * ty_123));
            stq0[0] = stq1[0] = 0.25f * (Grid_Vertex0[0] / (100.0f * sx_122));
...
u_int HH_Class_Prefix_Water_14(void) {
    u_long tex0;                                      // r2
    sceVif1Packet* pPk = HH_Vif1Packet_Current_Get(); // r16
    u_int result = 1;                                 // r2
    HH_Vif1PacketBuffer_GifTag_Open();
    tex0 = HH_Effect_Object_Texture_GS_Register_Tex0_Get(HH_WATER_14_TEX_ID, HH_WATER_14_CLUT_ID);
...
u_int HH_Class_Suffix_Water_14(void) {
    sceVif1Packet* pPk = HH_Vif1Packet_Current_Get(); // r16
    u_int result = 1;                                 // r2
    HH_Vif1PacketBuffer_GifTag_Open();
...
u_int HH_Class_Water_14(void* pBlock /* r2 */, ImpactQueue_Element* pElement /* r19 */) {
    ...
    switch (pThis->Step) { /* irregular */
        ...
        case HH_WATER_14_STEP_DRAW: {
            ...
            rad_omega = TO_RAD(degree_182) * pThis->Timer;
            if (rad_omega > 0.0f) {
            ...
            pThis->ST_Defference[0] = add_move_184 + ((center_185 + (base_move_183 * HH_MathWrapper_Cosf(rad))) / 512.0f);
            pThis->ST_Defference[1] = add_move_184 + ((center_185 + (base_move_183 * HH_MathWrapper_Sinf(rad))) / 512.0f);

            area = HH_Class_WaterCommon_WaveArea_CurrentArea_Search(pElement->Option.Vector[0], _Area_Info_List_0x0036FBF0, HH_WATER_14_TABLE_MAX);
            HH_Class_WaterCommon_Area_Enable_Table_Clear(pThis->Area_Enable_Table, HH_WATER_14_TABLE_MAX);
            if (area != -1) {
                HH_Class_WaterCommon_Area_Enable_Manager(pThis->Area_Enable_Table, _Area_Info_List_0x0036FBF0, HH_WATER_14_TABLE_MAX, area);
            } else {
                pThis->Area_Enable_Table[0] = 0;
            }
            if (pElement->Option.Int_Value[0] != 0) {
                int area = HH_Class_WaterCommon_WaveArea_CurrentArea_Search(pElement->Option.Vector[0], _Area_Info_List_0x0036FBF0, HH_WATER_14_TABLE_MAX);
                if (area != -1) {
                    Wave_Element wave_element; // r29+0x90
                    u_int x;
                    u_int z;
                    CurrentPosition_AreaIndex_Calculator(pElement, &_Area_Info_List_0x0036FBF0[area], &x, &z);
                    wave_element.Enable = 1;
                    wave_element.Area = area;
                    wave_element.Impact_Grid_Index[0] = x;
                    wave_element.Impact_Grid_Index[1] = z;
                    wave_element.Max_Distance0 = _distance_193;
                    wave_element.Lost_Time = _lost_time_194;
                    wave_element.Arrival = _arri_195;
                    wave_element.Omega = TO_RAD(_omega_196);
                    wave_element.Verocity = _v_197;
            ...
            {
                u_int i;
                for (i = 0; i < HH_WATER_14_TABLE_MAX; i++) {
                    if (pThis->Area_Enable_Table[i])
                        if ((pThis->Timer - pThis->Area_WavePostTime[i] > _interval_198)) {
                            WaveArea_Infomeation* pInfo = &_Area_Info_List_0x0036FBF0[i];
                            Wave_Element wave_element; // r29+0x90
                            ...
                            wave_element.Impact_Grid_Index[0] = x;
                            wave_element.Impact_Grid_Index[1] = z;
                            wave_element.Max_Distance0 = __distance_207;
                            wave_element.Lost_Time = __lost_time_208;
                            wave_element.Arrival = __arri_209;
                            wave_element.Omega = TO_RAD(__omega_210);
                            wave_element.Verocity = __v_211;
                            wave_element.Timer = 0;
                            HH_Class_WaterCommon_WaveElement_Addition(pThis->Wave_Info, 0x14, &wave_element);
            ...
            }
            {
                ...
                for (i = 0; i < HH_WATER_14_TABLE_MAX; i++) {
                    if (pThis->Area_Enable_Table[i] != 0) {
                        WaveArea_Infomeation* pInfo = _Area_Info_List_0x0036FBF0; // r21
                        WaveArea_Infomeation* pArea_Info = &pInfo[i];             // r2
                        float world_location[4];                                  // r29+0x150
