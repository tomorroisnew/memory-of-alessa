...
#include "Effect2/hh_class_water_13.h"
...
static u_int _area00_view_list_0x0036FA18[1] = {0};
static WaveArea_Infomeation _Area_Info_List_0x0036FA20[1] = {
    {/* .World_Location = */ {-101200.0f, -320.0f, -20600.0f, 1.0f},
     /* .Grid_WH = */ {2900.0f, 0.0f, 1300.0f, 100.0f},
     /* .Grid_Index = */ {29, 13},
     /* .pViewArea_List = */ &_area00_view_list_0x0036FA18,
     /* .ViewArea_List_Max = */ 1,
     /* .pGrid_Y_Value = */ NULL,
...
static u_int Object_Initialize(HH_Object_Water_13* pThis /* r16 */) {
    ...
    _Area_Info_List_0x0036FA20->pGrid_Y_Value = pThis->Area00_Grid_Y_Value;
    return result;
}
...
static u_int Object_Draw(HH_Object_Water_13* pThis /* r22 */, float* pGrid_Y_Value /* r21 */, float* WorldLocation /* r17 */, u_int Grid_X_Max /* r20 */, u_int Grid_Z_Max /* r18 */) {
    ...
    for (z_index = 0; z_index < z_grid_max; z_index++) {
        ...
        for (x_index = 0; x_index < Grid_X_Max; x_index++) {
            ...
            static float sx_112_0x0036FB60 = 2.5f; // @ 0x0036FB60
            static float ty_113_0x0036FB68 = 2.5f; // @ 0x0036FB68

            Grid_Vertex0[0] = Grid_Vertex1[0] = 100.0f * x_index;

            Grid_Vertex2[0] = ((x_index + 1) % x_grid_max) * 100.0f;
            Grid_Vertex0[1] = pGrid_Y[HH_WATER_13_GRID_INDEX_GET(x_index, z_index)];
            Grid_Vertex1[1] = pGrid_Y[HH_WATER_13_GRID_INDEX_GET(x_index, z_index + 1)];
            Grid_Vertex2[1] = pGrid_Y[HH_WATER_13_GRID_INDEX_GET((x_index + 1) % x_grid_max, z_index)];

            base = 0.25f * (1.0f / (100.0f * ty_113_0x0036FB68));
            stq0[0] = stq1[0] = 0.25f * (Grid_Vertex0[0] / (100.0f * sx_112_0x0036FB60));
...
u_int HH_Class_Prefix_Water_13(void) {
    ...
    u_int result = 1;                                 // r2
    HH_Vif1PacketBuffer_GifTag_Open();
    tex0 = HH_Effect_Object_Texture_GS_Register_Tex0_Get(HH_WATER_13_TEX_ID, HH_WATER_13_CLUT_ID);
...
u_int HH_Class_Water_13(void* pBlock /* r2 */, ImpactQueue_Element* pElement /* r19 */) {
    ...
    switch (pThis->Step) { /* irregular */
        ...
        case HH_WATER_13_STEP_DRAW: {
            ...
            pThis->ST_Defference[0] = add_move_174 + ((center_175 + (base_move_173 * HH_MathWrapper_Cosf(rad))) / 512.0f);
            pThis->ST_Defference[1] = add_move_174 + ((center_175 + (base_move_173 * HH_MathWrapper_Sinf(rad))) / 512.0f);

            area = HH_Class_WaterCommon_WaveArea_CurrentArea_Search(pElement->Option.Vector[0], _Area_Info_List_0x0036FA20, HH_WATER_13_TABLE_MAX);
            HH_Class_WaterCommon_Area_Enable_Table_Clear(pThis->Area_Enable_Table, HH_WATER_13_TABLE_MAX);
            if (area != -1) {
                HH_Class_WaterCommon_Area_Enable_Manager(pThis->Area_Enable_Table, _Area_Info_List_0x0036FA20, HH_WATER_13_TABLE_MAX, area);
            } else {
                pThis->Area_Enable_Table[0] = 0;
            }
            if (pElement->Option.Int_Value[0] != 0) {
                int area = HH_Class_WaterCommon_WaveArea_CurrentArea_Search(pElement->Option.Vector[0], _Area_Info_List_0x0036FA20, HH_WATER_13_TABLE_MAX);
                if (area != -1) {
                    Wave_Element wave_element; // r29+0x90
                    u_int x;
                    u_int z;
                    CurrentPosition_AreaIndex_Calculator(pElement, &_Area_Info_List_0x0036FA20[area], &x, &z);
                    wave_element.Enable = 1;
                    wave_element.Area = area;
                    wave_element.Impact_Grid_Index[0] = x;
                    wave_element.Impact_Grid_Index[1] = z;
                    wave_element.Max_Distance0 = _distance_183;
                    wave_element.Lost_Time = _lost_time_184;
                    wave_element.Arrival = _arri_185;
                    wave_element.Omega = TO_RAD(_omega_186);
                    wave_element.Verocity = _v_187;
            ...
            {
                u_int i;
                for (i = 0; i < HH_WATER_13_TABLE_MAX; i++) {
                    if (pThis->Area_Enable_Table[i])
                        if ((pThis->Timer - pThis->Area_WavePostTime[i] > _interval_188)) {
                            WaveArea_Infomeation* pInfo = &_Area_Info_List_0x0036FA20[i];
                            Wave_Element wave_element; // r29+0x90
                            ...
                            wave_element.Impact_Grid_Index[0] = x;
                            wave_element.Impact_Grid_Index[1] = z;
                            wave_element.Max_Distance0 = __distance_197;
                            wave_element.Lost_Time = __lost_time_198;
                            wave_element.Arrival = __arri_199;
                            wave_element.Omega = TO_RAD(__omega_200);
                            wave_element.Verocity = __v_201;
                            wave_element.Timer = 0;
                            HH_Class_WaterCommon_WaveElement_Addition(pThis->Wave_Info, 0x14, &wave_element);
            ...
            HH_DBG_Wrapper_T0_COUNT_Get();
            {
                ...
                for (i = 0; i < HH_WATER_13_TABLE_MAX; i++) {
                    if (pThis->Area_Enable_Table[i] != 0) {
                        HH_Class_WaterCommon_WaveArea_Calculator(&_Area_Info_List_0x0036FA20[i], pThis->Wave_Info, 0x14, i);
                    }
                }
                for (i = 0; i < HH_WATER_13_TABLE_MAX; i++) {
                    if (pThis->Area_Enable_Table[i] != 0) {
                        WaveArea_Infomeation* pInfo = _Area_Info_List_0x0036FA20; // r21
